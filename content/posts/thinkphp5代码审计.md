...
date: 2021-03-12
draft: false
image: https://cdn.jsdelivr.net/gh/Anthem-whisper/imgbed/img/20210312152311.jpeg
description: 
categories: 
- note
tags:
...
![](https://cdn.jsdelivr.net/gh/Anthem-whisper/imgbed/img/20210312150300.png)

洞其实不在这里

在路由调度的时候：

![image-20210224183737791](https://cdn.jsdelivr.net/gh/Anthem-whisper/imgbed/img/20210312150307.png)

跟进到thinkphp/library/think/route/dispatch/Module.php:

![image-20210224183857202](https://cdn.jsdelivr.net/gh/Anthem-whisper/imgbed/img/20210312150311.png)

跟进controller方法：(thinkphp/library/think/App.php)

![image-20210224193234503](https://cdn.jsdelivr.net/gh/Anthem-whisper/imgbed/img/20210312150328.png)

跟进parseModuleAndClass方法：(thinkphp/library/think/App.php)

![image-20210222221052520](https://cdn.jsdelivr.net/gh/Anthem-whisper/imgbed/img/20210312150405.png)

这个方法先对`$name`(类名)进行判断，当`$name`含有`\`时会直接将其作为类的命名空间路径，导致我们可以任意方法调用，比如：

* thinkphp/library/think/Container.php：

![image-20210223112805010](https://cdn.jsdelivr.net/gh/Anthem-whisper/imgbed/img/20210312150430.png)
...
![image-20210223113330871](https://cdn.jsdelivr.net/gh/Anthem-whisper/imgbed/img/20210312150418.png)

因为是private，所以查找同一类里面的用例：

1. Request::input方法：

![image-20210223113649493](https://cdn.jsdelivr.net/gh/Anthem-whisper/imgbed/img/20210312150443.png)
...
![image-20210223114040419](https://cdn.jsdelivr.net/gh/Anthem-whisper/imgbed/img/20210312150457.png)
...
![image-20210223130238959](https://cdn.jsdelivr.net/gh/Anthem-whisper/imgbed/img/20210312150511.png)
...
![image-20210224104838156](https://cdn.jsdelivr.net/gh/Anthem-whisper/imgbed/img/20210312150528.png)

原理类似，在`App::run()`方法里面，`Loader::controller`进行调度的时候，当`$name`含有`\`时会直接将其作为类的命名空间路径，导致我们可以任意方法调用。比如：

* thinkphp/library/think/App.php:

![image-20210224111256654](https://cdn.jsdelivr.net/gh/Anthem-whisper/imgbed/img/20210312150543.png)
...
![](https://cdn.jsdelivr.net/gh/Anthem-whisper/imgbed/img/20210416105559.png)
...
![image-20210308164140184](https://cdn.jsdelivr.net/gh/Anthem-whisper/imgbed/img/20210312150559.png)

可以看到有一个可以控制的函数名`$_POST[Config::get['var_method']`，而`var_method`的值在application/config.php里面为`_method`:

![image-20210308164849228](https://cdn.jsdelivr.net/gh/Anthem-whisper/imgbed/img/20210312150613.png)

于是可以POST传入`_method`改变`$this->{$this->method}($_POST);`达到任意调用此类中的方法

而如果调用此类中的`__construct`方法：
![image-20210308165349746](https://cdn.jsdelivr.net/gh/Anthem-whisper/imgbed/img/20210312150623.png)

有一个foreach，可以引起POST数据对Requests对象属性的变量覆盖。

在App::run()方法里面，如果我们开启了debug模式，则会调用Request::param()方法：

![image-20210309112057671](https://cdn.jsdelivr.net/gh/Anthem-whisper/imgbed/img/20210312150633.png)

当然，即使没有开启debug，在App::run()里面的调用的exec方法同样也会调用Request::param()方法

![image-20210309115557269](https://cdn.jsdelivr.net/gh/Anthem-whisper/imgbed/img/20210312150643.png)

因为调用栈太深，就不一个个跟了

![image-20210309115705097](https://cdn.jsdelivr.net/gh/Anthem-whisper/imgbed/img/20210312150651.png)
...
![image-20210309113026498](https://cdn.jsdelivr.net/gh/Anthem-whisper/imgbed/img/20210312150703.png)
...
![](https://cdn.jsdelivr.net/gh/Anthem-whisper/imgbed/img/20210312150713.png)
...
![image-20210417135947010](https://cdn.jsdelivr.net/gh/Anthem-whisper/imgbed/img/20210417135956.png)
...
![image-20210417144037447](https://cdn.jsdelivr.net/gh/Anthem-whisper/imgbed/img/20210417144045.png)

利用这一点，我们可以传入一个对象来触发`__toString`方法，于是全局搜索`__toString`

![image-20210417194627181](https://cdn.jsdelivr.net/gh/Anthem-whisper/imgbed/img/20210417202754.png)
...
![image-20210417202816283](https://cdn.jsdelivr.net/gh/Anthem-whisper/imgbed/img/20210417211525.png)
...
![](https://cdn.jsdelivr.net/gh/Anthem-whisper/imgbed/img/20210417212035.png)
...
![](https://cdn.jsdelivr.net/gh/Anthem-whisper/imgbed/img/20210417222219.png)
...
![](https://cdn.jsdelivr.net/gh/Anthem-whisper/imgbed/img/20210418113603.png)

第一条路，符合if判断之后`$value`可控，得到返回值`$values`之后代码跳出getRelationDate方法，运行至条件3处

![](https://cdn.jsdelivr.net/gh/Anthem-whisper/imgbed/img/20210418110614.png)

`if (method_exists($modelRelation, 'getBindAttr'))`，发现在HasOne的父类OneToOne里面是可控的：

![](https://cdn.jsdelivr.net/gh/Anthem-whisper/imgbed/img/20210417212650.png)
...
![](https://cdn.jsdelivr.net/gh/Anthem-whisper/imgbed/img/20210418141551.png)

于是我们可以寻找其他的方法，比如Output类的`__call`方法

![](https://cdn.jsdelivr.net/gh/Anthem-whisper/imgbed/img/20210418143247.png)

这里调用了block方法，跟进：

![](https://cdn.jsdelivr.net/gh/Anthem-whisper/imgbed/img/20210418143440.png)

继续跟进：

![](https://cdn.jsdelivr.net/gh/Anthem-whisper/imgbed/img/20210418143521.png)

疯狂跟进：

![](https://cdn.jsdelivr.net/gh/Anthem-whisper/imgbed/img/20210418143650.png)

`$this->handle`是可控的，继续全局搜索write，寻找可控的点，找到了/thinkphp/library/think/session/driver/Memcached.php

![](https://cdn.jsdelivr.net/gh/Anthem-whisper/imgbed/img/20210418143918.png)

同样`$this->handle`可控，继续全局搜索set，找到thinkphp/library/think/cache/driver/File.php

![](https://cdn.jsdelivr.net/gh/Anthem-whisper/imgbed/img/20210418144434.png)

可以`file_put_contents`写shell，并且`$filename`在getCacheKey方法当中是可控的，伪协议绕一下exit就可以了

![](https://cdn.jsdelivr.net/gh/Anthem-whisper/imgbed/img/20210418145126.png)

但是`$data`比较麻烦，他从传入的`$value`取值，`set`方法中的参数来自先前调用的`write`方法，`write`之前在writeln的时候就传入了true，不可控。偷一张图：

![](https://cdn.jsdelivr.net/gh/Anthem-whisper/imgbed/img/20210418145636.png)

继续跟进后面的setTagItem方法，我们可以看到它再次调用了set方法:

![](https://cdn.jsdelivr.net/gh/Anthem-whisper/imgbed/img/20210418145844.png)
